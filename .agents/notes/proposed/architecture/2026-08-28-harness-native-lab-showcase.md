...
The Host remains authoritative for registered directory Workspaces, laboratory Projects, Experiments, Runs, Artifact manifests and Project files. A Project links to exactly one directory Workspace without replacing Workspace identity, and a Workspace maps to at most one Project: repeated creation resolves the existing mapping, while an unmapped Workspace creates a Project named from its directory. Project scope mutations go through the Host adapter action rather than a page-specific Facade call. A Session remains a Harness conversation and records explicit `created`, `continued` or `reviewed` links to an Experiment. An Experiment retains multiple immutable Runs, and a retry creates a new Run with provenance. The right Project workspace groups authorized Project files as configuration, conversation output and run artifacts. Host writes append a durable Project-scoped file event containing metadata and revision only; the open file catalog reloads from that event. Browser state stores presentation selection and reloads records through typed Facade commands; it never owns absolute paths or file bodies.
...
The assembled LABWEAVE Web profile preserves `conversationMode` and default-selection metadata, uses one shared Conversation presentation with the native Harness header, hero and composer, and mounts the Project workspace in the right details column. The Host-composed keyless journey is covered by service and composition tests for Project/Session identity, scoped context, Workflow/Skill/Plan proposal and approval, Runtime execution, replanning, Project files, verdict and report persistence.

The existing browser scenario covers the dynamic global monitor, Projects tree, Project lifecycle destinations, lifecycle-first Overview, pending-action presentation, configuration capability statuses, typed Project and Artifact selection, one textarea, draft retention while changing destinations, Agent timeline expansion, sidebar rail behavior, and responsive layouts. The remaining browser acceptance must cover both panel collapse paths, central conversation scrolling, Project/file switching, metadata-event refresh, manual refresh, preview and download. The repeatable assembled browser scenario is `apps/web/tests/lab-showcase.e2e.ts`; browser evidence for the Stage 9 Knowledge-to-report business flow remains outside this record.

The client fixture covers grouped Project-file metadata, authorized preview and download actions, and metadata-only revision events that reload the active catalog. The production composition uses a typed Host adapter for Project, Run, report, Run actions, Project-file commands and events, capability summaries, and Agent/Runtime event projections. The assembled browser gate remains open until the responsive interaction evidence is collected on a browser-capable device.
