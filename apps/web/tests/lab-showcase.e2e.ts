...
describe('web e2e: LABWEAVE showcase composition', () => {
  ...
  it('traverses the three-pane Project shell and one shared Agent conversation', async () => {
    ...
    await expect(projectNavigation.getByRole('button', { name: 'Overview', exact: true })).toBeVisible()
    await expect(projectNavigation.getByRole('button', { name: 'Planning and Workflow', exact: true })).toBeVisible()
    await expect(projectNavigation.getByRole('button', { name: 'Plan approval', exact: true })).toBeVisible()
    await expect(projectNavigation.getByRole('button', { name: 'Execution monitoring', exact: true })).toBeVisible()
    await expect(projectNavigation.getByRole('button', { name: 'Step orchestration', exact: true })).toBeVisible()
    await expect(projectNavigation.getByRole('button', { name: 'Evidence and reports', exact: true })).toBeVisible()
    await expect(projectNavigation.getByRole('button', { name: 'Project files', exact: true })).toBeVisible()
    await expect(projectNavigation.getByRole('button', { name: 'Archive', exact: true })).toBeVisible()
    await expect(page.locator('[data-lab-project-navigation]')).toHaveCount(1)
