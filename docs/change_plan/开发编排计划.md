...
本文是 [实施方案](实施方案.md) 的长期路线图，规定开发顺序、任务依赖、每块交付物和退出条件。架构职责、证据边界和领域概念由实施方案定义；当前第一轮的施工位置、并行边界和完成条件由 [第一轮敏捷开发规划](第一轮敏捷开发规划.md) 维护，实时任务状态由 OpenSpec 维护。
...
| 当前交付 | 覆盖的路线图任务 | 责任 |
| --- | --- | --- |
| 已完成基础闭环 | TASK-LAB-000、TASK-KNOW-100、TASK-PROTO-200、TASK-DEVICE-300、TASK-PROTO-210、TASK-DEVICE-310、TASK-RUN-400 的首轮范围 | 提供领域类型、Service/Provider/Consumer、Mock 设备、Runtime、typed Facade 和最小 Web Consumer |
| [`pdf-knowledge-parser`](../../openspec/changes/pdf-knowledge-parser/proposal.md) | TASK-KNOW-110 的 PDF 范围、TASK-KNOW-120、TASK-UI-115、TASK-UI-125 和 TASK-PROTO-220 的知识策展部分 | 完成 PDF/CSV/TSV/文本录入、PDF 解析、SOP 审核发布、本地索引召回和 Knowledge 工作区；DOCX 保留在长期任务范围 |
| [`lab-harness-native-workspace`](../../openspec/changes/lab-harness-native-workspace/proposal.md) | TASK-UI-320、TASK-UI-410、TASK-AGENT-500、TASK-UI-520 和 TASK-AGENT-510 的第一轮交互范围 | 完成 Harness 原生侧栏、项目多 Session、Agent 对话、计划/Skill 人工确认和运行反馈 |
| I5 联合验收 | M2 与 M3 的首轮交汇 | 验证项目范围内的已发布知识召回、计划确认、Mock 执行、结果验证和报告连续可操作 |

这两条 OpenSpec 变更不新增路线图层级。知识线生产稳定 Consumer DTO、能力状态、opaque ID 和独立 Knowledge contributor；Harness 工作区线只消费并挂载这些接口。共享 `lab-mvp-web` 入口只组合独立 Knowledge 与 Project/Conversation 协议模块，避免并行会话修改同一业务实现。
...
本文件只维护长期依赖，不复制实时完成数量。当前施工状态以 `openspec list` 和两个 active change 的 `tasks.md` 为唯一来源，完成证明进入对应测试、OpenSpec verify 和 Agent Note。

| 交付块 | 状态类型 | 下一阶段条件 |
| --- | --- | --- |
| 基础能力与最小 Web Consumer | 已完成的第一轮基线 | 继续保持现有 Service、Runtime 和 `/api/lab` 合同可用 |
| PDF、数据录入与召回 | Active OpenSpec | 独立完成 Knowledge 工作区与合同测试，发布兼容的 Consumer fixture |
| Harness 原生工作区与 Agent 闭环 | Active OpenSpec | 在 fake Consumer 下通过项目/多 Session/计划确认/运行卡片验收 |
| I5 联合验收 | 等待两线独立阶段门 | 真实 Knowledge contributor、项目 scope、Agent 规划和 Runtime 组合 smoke 通过 |
| TASK-EVAL-600、TASK-UI-610、TASK-HW-900 | 后续路线图 | 第一轮 Definition of Done 完成后分别立项 |

## 9. 第一开发块

当前开发块由两条 OpenSpec 线并行组成：`pdf-knowledge-parser` 和 `lab-harness-native-workspace`。两线先完成共享 Knowledge Consumer fixture 和独立协议模块，再各自按 2–5 个工作日的可演示切片施工；任何一方不能通过直接读取另一方数据库、复制 UI、使用固定 Agent 提示按钮或写死测试资料绕过依赖。

两线达到独立阶段门后只进入一次 I5 联合集成：挂载 Knowledge contributor，将 source/version scope 绑定到项目，通过 Harness Session 召回已发布 SOP，生成并人工确认计划，使用现有 Mock Device/Runtime 执行、验证并生成报告。该路径通过前，不进入 TASK-EVAL-600、真实设备或生产权限建设。
