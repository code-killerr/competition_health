...
The active `pdf-knowledge-parser` change owns source upload, PDF/CSV/TSV/text ingestion, parser lifecycle, SOP curation, indexing, embeddings, retrieval and the Knowledge workspace body. This change consumes its stable Service/Facade records and citation identities; it does not import Knowledge Provider internals or duplicate its browser workflow.
...
- Provide global Knowledge, Devices and Projects navigation while mounting the Knowledge workspace supplied by the PDF/knowledge change.
...
- No PDF parsing, file transport, document indexing, embedding adapter, retrieval ranking, SOP curation or Knowledge workspace implementation; those belong to `pdf-knowledge-parser`.
...
The Web layer continues to send typed commands to `lab-mvp-web`; browser code never imports a Provider, SQLite module or Agent tool implementation. The project layer stores only stable source/version/citation references returned by the Knowledge Facade. Knowledge workspace rendering and writes remain owned by `pdf-knowledge-parser`; this change mounts that contribution and supplies selected project scope to Agent context and search requests.

Parallel implementation keeps project/session/Agent commands in a dedicated project-conversation protocol module and keeps Harness workspace UI in its own contributor/package. It does not edit the Knowledge protocol module or Knowledge workspace contributor. The shared Web protocol entry only composes exports and registrations and is updated at the integration checkpoint.
...
**Alternative considered: duplicate Knowledge upload/search panels inside the project client.** Rejected because `pdf-knowledge-parser` owns that workflow and exposes a public workspace contribution plus typed Facade commands.
...
- [The Knowledge contract changes while both lines are under development] → Pin shared DTOs and capability status first; current-line tests use a fake Knowledge Consumer and never import the Provider.
...
- [Knowledge data may be unavailable while the parallel change is incomplete] → Render an explicit capability-unavailable state and allow project/session/Agent UI tests to run against deterministic fixtures.

## Migration Plan

1. Freeze the shared Knowledge Consumer DTOs and capability-unavailable response with `pdf-knowledge-parser`; neither line imports the other's Provider or UI implementation.
...
5. Run a joint composed smoke after the PDF/knowledge line publishes its Facade and workspace contribution.
