 ...
 * ctx.layout is the cross-plugin panel and root application-view contract;
 * application selection lives with this layout store. A second effect seats the theme
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => {
    ...
    const disposeAppViews = layout.attachAppViews(ctx.slots)
    return () => {
      disposeAppViews()
      disposeRegistration()
