...
describe('lab Web SOP protocol', () => {
  it('parses a cited SOP draft, fact confirmation and publication command', () => {
    ...
    expect(parseLabWebCommand({ command: 'knowledge-fact-confirm', citationId: 'citation-block-1', confirmedBy: 'reviewer', note: 'checked' })).toMatchObject({
      command: 'knowledge-fact-confirm',
      citationId: 'citation-block-1',
  ...
  it('rejects incomplete SOP steps and blank publication actors', () => {
    ...
    expect(() => parseLabWebCommand({ command: 'knowledge-fact-confirm', citationId: ' ', confirmedBy: 'reviewer' })).toThrow(/citationId/)
