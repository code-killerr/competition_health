...
// 占位图：使用内联 SVG data URI 作为实验影像占位
const placeholder = (label: string, hue: number) =>
  `data:image/svg+xml;utf8,${encodeURIComponent(
    `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="300"><rect width="400" height="300" fill="hsl(${hue},30%,12%)"/><rect x="20" y="20" width="360" height="260" fill="none" stroke="hsl(${hue},40%,30%)" stroke-width="2" rx="8"/><circle cx="200" cy="150" r="60" fill="hsl(${hue},50%,20%)" stroke="hsl(${hue},60%,45%)" stroke-width="2"/><text x="200" y="155" text-anchor="middle" fill="hsl(${hue},60%,60%)" font-family="monospace" font-size="16">${label}</text></svg>`
  )}`;
...
export const stageEvidence: StageEvidence[] = [
  {
    ...
    processImage: placeholder('SeekOne DD 设备', 168), processSource: 'SeekOne DD p.14',
    resultLabel: '设备状态识别', resultCaption: '设备、夹具和程序入口均可回链到设备注册表。',
    resultImage: placeholder('运行界面', 217), resultSource: 'SeekOne DD p.14',
  ...
  },
  {
    ...
    processImage: placeholder('组织贴片', 168), processSource: 'SeekSpace p.26',
    resultLabel: '贴片覆盖识别', resultCaption: '组织边界清晰，未见明显脱落或超出标记区的阻断异常。',
    resultImage: placeholder('组织边界', 168), resultSource: 'SeekSpace p.26',
  ...
  },
  {
    ...
    processImage: placeholder('荧光成像', 168), processSource: 'SeekSpace p.31',
    resultLabel: 'DAPI 组织图像', resultCaption: '组织主体和边界被识别，图像可进入空间定位与分割流程。',
    resultImage: placeholder('DAPI 组织', 168), resultSource: 'SeekSpace p.31',
  ...
  },
  {
    ...
    processImage: placeholder('细胞计数', 168), processSource: 'SeekSpace p.35',
    resultLabel: '核悬液 QC 结果', resultCaption: '识别计数区域，并将 82% 有核率、16% 结团率回写规则门。',
    resultImage: placeholder('核悬液 QC', 168), resultSource: 'SeekSpace p.35',
  ...
  },
  {
    ...
    processImage: placeholder('Chip S3 加样', 217), processSource: 'SeekSpace p.41',
    resultLabel: 'DD 运行状态', resultCaption: 'Chip Holder 与空间转录组程序状态通过，结果回链到运行 ID。',
    resultImage: placeholder('DD 运行界面', 217), resultSource: 'SeekOne DD p.14',
  ...
  },
  {
    ...
    processImage: placeholder('建库产物', 217), processSource: 'SeekOne DD p.14',
    resultLabel: '文库峰图 QC', resultCaption: '识别主峰 174–437 bp 区间及整体峰型，进入文库放行。',
    resultImage: placeholder('文库峰图', 168), resultSource: 'SeekOne DD p.20',
  ...
  },
  {
    ...
    processImage: placeholder('空间结果输入', 168), processSource: 'SeekSpace p.31',
    resultLabel: '空间定位结果', resultCaption: '组织边界、空间区域和下游矩阵产物完成关联。',
    resultImage: placeholder('空间定位结果', 168), resultSource: 'SeekSpace p.31',
